...
import {
  CategoryMasterFormat,
  PaymentTableFormat,
  BudgetDisplayCategoryMasterFormat,
  BudgetMasterFormat,
} from "./SheetFormat";
...
export class GoogleSheetOperator implements SheetOperator {
  ...
  async fetchCategories(): Promise<Category[]> {
    ...
    const headers = values[0];
    const categoryIdIndex = headers.indexOf(
      CategoryMasterFormat.headerCategoryID,
    );
  ...
  addYearMonth(
    ...
    // 2. 全てを「総月数」に変換
    const totalMonths = srcYear * 12 + srcMonthIndex + addYear * 12 + addMonth;
  ...
  async fetchSheetQuery(
    tableName: string,
    query: string,
    headerLineNo: number = 1,
  ): Promise<Response> {
  ...
  async fetchPaymentsOrderByDateAsc(
    year: number,
    month: number,
  ): Promise<Payment[]> {
    const paymentHeaderColIndex = await this.fetchTableHeaderColumnIndex(
      PaymentTableFormat.title,
    );

    const paymentIDColNo =
      paymentHeaderColIndex[PaymentTableFormat.headerPaymentID] + 1;
    const dateColNo =
      paymentHeaderColIndex[PaymentTableFormat.headerPaymentDate] + 1;
    const titleColNo =
      paymentHeaderColIndex[PaymentTableFormat.headerTitle] + 1;
    const categoryIDColNo =
      paymentHeaderColIndex[PaymentTableFormat.headerCategoryID] + 1;
    const amountColNo =
      paymentHeaderColIndex[PaymentTableFormat.headerAmount] + 1;
  ...
  async requestDeletePayment(paymentID: string): Promise<void> {
    ...
    const paymentIDColNo =
      paymentHeaderColIndex[PaymentTableFormat.headerPaymentID] + 1;
    const rowNoColNo =
      paymentHeaderColIndex[PaymentTableFormat.headerRowNo] + 1;
    ...
    const findIDResponse = await this.fetchSheetQuery(
      PaymentTableFormat.title,
      findIDQuery,
    );
    ...
    await this.requestDeleteRow(
      PaymentTableFormat.title,
      Number(findIDRows[0][0]),
    );
  }
  ...
  async computeBalance(
    ...
    const fetchBudgetDisplayCategories = async () => {
      ...
      const idxCategoryID =
        colIndexMap[BudgetDisplayCategoryMasterFormat.headerCategoryID];
      const idxDisplayOrder =
        colIndexMap[BudgetDisplayCategoryMasterFormat.headerDisplayOrder];
    ...
    const { year: lastMonthTargetYear, month: lastMonthTargetMonth } =
      this.addYearMonth(targetYear, targetMonth, 0, -1);
    ...
    const paymentColNoHeaderCategoryID =
      paymentHeaderColIndex[PaymentTableFormat.headerCategoryID] + 1;
    const paymentColNoHeaderPaymentDate =
      paymentHeaderColIndex[PaymentTableFormat.headerPaymentDate] + 1;
    const paymentColNoHeaderAmount =
      paymentHeaderColIndex[PaymentTableFormat.headerAmount] + 1;

    if (
      paymentColNoHeaderCategoryID === undefined ||
    ...
    const budgetMasterHeaderColIndex = await this.fetchTableHeaderColumnIndex(
      BudgetMasterFormat.title,
    );

    const budgetColNoHeaderCategoryID =
      budgetMasterHeaderColIndex[BudgetMasterFormat.headerCategoryID] + 1;
    const budgetColNoHeaderTargetYearMonth =
      budgetMasterHeaderColIndex[BudgetMasterFormat.headerTargetYearMonth] + 1;
    const budgetColNoHeaderBudgetAmount =
      budgetMasterHeaderColIndex[BudgetMasterFormat.headerBudgetAmount] + 1;

    if (
      budgetColNoHeaderCategoryID === undefined ||
    ...
    for (const categoryID of budgetDisplayCategories) {
      ...
      const lastBudgetAmount =
        lastCategoryIDtoBudgetAmount.get(categoryID) ?? 0;
  ...
  async propagateLatestBudgetUntilTarget(
    ...
    const budgetColNoHeaderBudgetAmount =
      budgetMasterHeaderColIndex[BudgetMasterFormat.headerBudgetAmount] + 1;
    const budgetColNoRowNo =
      budgetMasterHeaderColIndex[BudgetMasterFormat.headerRowNo] + 1;

    if (
      budgetColNoHeaderCategoryID === undefined ||
      budgetColNoHeaderTargetYearMonth === undefined ||
      budgetColNoHeaderBudgetAmount === undefined ||
      budgetColNoRowNo === undefined
    ) {
      throw new Error("必要なヘッダが存在しません");
    }

    const rowsBudgetOrderByDateAsc =
      await this.selectTableInPeriodOrderByDateAsc(
        BudgetMasterFormat.title,
        this.columnNoToAlphabet(budgetColNoHeaderTargetYearMonth),
        undefined,
        undefined,
    ...
    for (const row of rowsBudgetOrderByDateAsc) {
      const budgetCategoryID = row[budgetColNoHeaderCategoryID - 1];
      const budgetAmount = Number(row[budgetColNoHeaderBudgetAmount - 1]);

      const budgetDate = this.parseGvizDate(
        row[budgetColNoHeaderTargetYearMonth - 1],
      );
      const budgetYear = budgetDate.getFullYear();
      const budgetMonth = budgetDate.getMonth() + 1;

      categoryIDtoLatestBudget.set(budgetCategoryID, {
        year: budgetYear,
        month: budgetMonth,
        budgetAmount,
      });
    ...
    for (const [
      categoryID,
      latestBudget,
    ] of categoryIDtoLatestBudget.entries()) {
      if (
        latestBudget.year === targetYear &&
        latestBudget.month === targetMonth
      ) {
        continue;
      }
      ...
      for (let i = 0; i < diffMonthCount; i++) {
        const { year: nextYear, month: nextMonth } = this.addYearMonth(
          currentYear,
          currentMonth,
          0,
          1,
        ...
        row[budgetColNoRowNo - 1] = this.rowNoFunction;
  ...
  async requestAddCategory(name: string): Promise<void> {
    const categoryMasterHeaderColIndex = await this.fetchTableHeaderColumnIndex(
      CategoryMasterFormat.title,
    );
    const categoryIDColIndex =
      categoryMasterHeaderColIndex[CategoryMasterFormat.headerCategoryID];
    const categoryNameColIndex =
      categoryMasterHeaderColIndex[CategoryMasterFormat.headerName];
    const categoryRowNoColIndex =
      categoryMasterHeaderColIndex[CategoryMasterFormat.headerRowNo];

    if (
      categoryIDColIndex === undefined ||
  ...
  async requestDeleteCategory(categoryID: string): Promise<void> {
    ...
    const categoryIDColNo =
      categoryMasterHeaderColIndex[CategoryMasterFormat.headerCategoryID] + 1;
    const categoryRowNoColNo =
      categoryMasterHeaderColIndex[CategoryMasterFormat.headerRowNo] + 1;
  ...
  async updateCategory({ categoryID, name }: Category): Promise<void> {
    ...
    const categoryIDColNo =
      categoryMasterHeaderColIndex[CategoryMasterFormat.headerCategoryID] + 1;
    const categoryNameColNo =
      categoryMasterHeaderColIndex[CategoryMasterFormat.headerName] + 1;
    const categoryRowNoColNo =
      categoryMasterHeaderColIndex[CategoryMasterFormat.headerRowNo] + 1;
    ...
    await this.requestUpdateRows(
      CategoryMasterFormat.title,
      this.columnNoToAlphabet(categoryNameColNo),
      Number(findIDRows[0][0]),
      [[name]],
    );
  }
  ...
  async fetchOrderedBudgetDisplayCategoryIDs(): Promise<string[]> {
    const budgetDisplayHeaderColIndex = await this.fetchTableHeaderColumnIndex(
      BudgetDisplayCategoryMasterFormat.title,
    );
    const categoryIDColNo =
      budgetDisplayHeaderColIndex[
        BudgetDisplayCategoryMasterFormat.headerCategoryID
      ] + 1;
    const orderColNo =
      budgetDisplayHeaderColIndex[
        BudgetDisplayCategoryMasterFormat.headerDisplayOrder
      ] + 1;
    ...
    const response = await this.fetchSheetQuery(
      BudgetDisplayCategoryMasterFormat.title,
      query,
    );
  ...
  async updateOrderedBudgetDisplayCategories(
    categoryIDs: string[],
  ): Promise<void> {
    const budgetDisplayHeaderColIndex = await this.fetchTableHeaderColumnIndex(
      BudgetDisplayCategoryMasterFormat.title,
  ...
  async requestAddDisplayBudget(categoryID: string): Promise<void> {
    ...
    const rowNoColIndex =
      budgetDisplayHeaderColIndex[
        BudgetDisplayCategoryMasterFormat.headerRowNo
      ];

    if (
      categoryIDColIndex === undefined ||
      orderColIndex === undefined ||
    ...
    const maxOrderQuery = `
        ...
        `;
    const maxOrderResponse = await this.fetchSheetQuery(
      BudgetDisplayCategoryMasterFormat.title,
      maxOrderQuery,
    );
    ...
    await this.requestAddRowsToTable(BudgetDisplayCategoryMasterFormat.title, [
      row,
    ]);
  }
  ...
  async fetchBudgets(year: number, month: number): Promise<Budget[]> {
    const budgetHeaderColIndex = await this.fetchTableHeaderColumnIndex(
      BudgetMasterFormat.title,
    );
    const categoryIDColNo =
      budgetHeaderColIndex[BudgetMasterFormat.headerCategoryID] + 1;
    const dateColNo =
      budgetHeaderColIndex[BudgetMasterFormat.headerTargetYearMonth] + 1;
    const budgetAmountColNo =
      budgetHeaderColIndex[BudgetMasterFormat.headerBudgetAmount] + 1;
  ...
  async updateBudget(
    ...
    await this.requestUpdateRows(
      BudgetMasterFormat.title,
      this.columnNoToAlphabet(budgetAmountColNo),
      Number(findIDRows[0][0]),
      [[budgetAmount.toString()]],
    );
  }

  async requestAddBudget(
    year: number,
    month: number,
    categoryID: string,
    budgetAmount: number,
    ...
    const categoryIDColIndex =
      budgetHeaderColIndex[BudgetMasterFormat.headerCategoryID];
    const dateColIndex =
      budgetHeaderColIndex[BudgetMasterFormat.headerTargetYearMonth];
    const budgetAmountColIndex =
      budgetHeaderColIndex[BudgetMasterFormat.headerBudgetAmount];
